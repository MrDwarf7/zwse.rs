...
#   ZWSE_VERSION   - version tag to install (default: latest)
#   ZWSE_INSTALL   - install dir (default: /usr/local/bin or ~/.local/bin)
#   GITHUB_TOKEN                    - avoids API rate limits in CI

set -e

REPO="MrDwarf7/zwse.rs"
RELEASES_URL="https://github.com/$REPO/releases"
BIN="zwse"
INSTALL_DIR="$ZWSE_INSTALL:-}"
...
fetch_latest_version() {
    ...
    [ -n "$tag" ] || die "could not resolve latest release. Install via \`cargo install zwse\` instead."
    echo "$tag"
}
...
# Artifact name per draft.yml: zwse-<target>-<tag>.zip
artifact="zwse-${target}-${version}.zip"
...
echo ""
echo "  zwse $ver installed. Run \`zwse --help\` to get started."